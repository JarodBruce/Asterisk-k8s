...
現在は
```
[HGW(192.168.0.1/24)]→[Asterisk(192.168.0.2/24,192.168.1.200/24]→[IP電話]
```
の構成になっていますが、
将来的にAsteriskをk8sに移植したいと考えています。
その際にHGWのNATしていたりHGWの内線設定がMACアドレスで行われていてIPアドレスではないなど不都合なことが多いです
なので
```
[HGW(192.168.0.1/24)]→[ラズパイ(192.168.0.10/24,192.168.1.201)]→[Asterisk(192.168.1.200)]→[IP電話]
```
の構成に移行したいと考えています。
なのでラズパイにはHGWと他のネットワークとのアダプタになって欲しいと考えています。
実現には具体的にどんなものを実行すればいいですか？

またこのPCはMACでありテスト等は行わないでください。
